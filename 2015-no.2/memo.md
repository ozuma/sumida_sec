...
- ポートスキャンを隠す
    - スキャンするポートを絞る。65535ポートやる必要は普通は無い
...
下の表は、様々なホストで利用されている開放ポートを集計し、有効性(カバレッジと言った方がいいかな)をカバーするためにはいくつのポートをスキャンすれば良いかを計算したもの。この数だけ、上位順にポートを選んでいけば良い。

たとえば1ホストごとに90%の開放ポートをカバーするように設定するには、上位576個のポートをスキャンすべきということが分かる。
...
引用：NMAP NETWORK SCANNING, Gordon "Fyodor" Lyon (2008) いわゆる「目ン玉本」、公式ガイドブック

スキャンするポートを絞れば絞るほど、攻撃は見つかりにくい原則を思いだそう。カバレッジ率を考えると、65536個のポートをスキャンする意味はほとんど無い。何もオプションを指定しない場合、デフォルトでは上位1000個のポート(カバレッジ90%チョイくらい)になる。さらに100個のポートをスキャンする(`-F`オプション)方が当然見つかりにくいし、それでも75%くらいのカバレッジになる。

上位nポートをスキャンしたい場合は、`--top-ports`オプションを使うと良い。

```
root@kali:~# nmap -n -v --top-ports 10 192.168.2.66
```
...
- data配下のkeyについて、本物のsshdのもの()で上書きすれば区別付かなくなる。しかし本末転倒感がある。
- なお、kippoのkeyは最終行に改行が無いとエラーになるので、本物のキーをコピーしてから最終行の改行を消す必要がある。

```
$ cat ssh_host_dsa_key | ruby -0777 -ne 'print $_.chomp("")'
```
